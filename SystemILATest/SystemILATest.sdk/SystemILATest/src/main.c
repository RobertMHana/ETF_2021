...
#define INJECTOR_BASE_ADDR 0x7AA00000
...
int XDmaPs_Example_W_IntrInjector(XScuGic *GicPtr, u16 DeviceId, int *SourceAddress, int offset, int  *DestinationAddress, int size, int burstlength);
...
int main()
{
	...
	status = XDmaPs_Example_W_Intr(&GicInstance, DMA_DEVICE_ID, &injectorBuffer[0], INJECTOR_BASE_ADDR);
			if (status != XST_SUCCESS) {
    ...
    for( i = 1; i <= 8192; i*=2) //number of 32 bit words to transfer
    {
				for(h = 4; h <= 16; h*=2) //burst size
				{
					...
					while(timerValue > 0){
						status = XDmaPs_Example_W_IntrInjector(&GicInstance, DMA_DEVICE_ID, &injectorBuffer[0], 0, INJECTOR_BASE_ADDR, size, h);
							if (status != XST_SUCCESS) {
								xil_printf("\n\rError: XDMaPs_Example_W_IntrAlt on Injector DMA Fill, failed");
								//return XST_FAILURE;
							}
						while(DmaDone == 0);
						DmaDone = 0;
...
int XDmaPs_Example_W_IntrInjector(XScuGic *GicPtr, u16 DeviceId, int *SourceAddress, int offset, int  *DestinationAddress, int size, int burstlength)
{
	...
	DmaCmd.ChanCtrl.DstInc = 0;			      //the destination is the Injector does not increment.
	DmaCmd.BD.SrcAddr = (u32 *) newSource;
	DmaCmd.BD.DstAddr = (u32 *) DestinationAddress;
...
void captureILA(void){
	...
	while(1){
		...
		for(j = 0; j < 20000000; j++){ k = k + 1;}
		status = XDmaPs_Example_W_IntrInjector(&GicInstance, DMA_DEVICE_ID, &injectorBuffer[0], 0, INJECTOR_BASE_ADDR, 16, 4);
		if (status != XST_SUCCESS) {
			xil_printf("\n\rError: XDMaPs_Example_W_IntrAlt on Injector DMA Fill, failed");
			return XST_FAILURE;
		}
