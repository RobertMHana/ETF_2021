...
XStatus AXISAMPLERWITHFIFO_Reg_SelfTest(void * baseaddr_p)
{
	...
	for (write_loop_index = 0 ; write_loop_index < 4; write_loop_index++)
	  AXISAMPLERWITHFIFO_mWriteReg (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
	for (read_loop_index = 0 ; read_loop_index < 4; read_loop_index++)
	  if ( AXISAMPLERWITHFIFO_mReadReg (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
...
XStatus AXISAMPLERWITHFIFO_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  AXISAMPLERWITHFIFO_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = AXISAMPLERWITHFIFO_mReadMemory(baseaddr+4*Index);
